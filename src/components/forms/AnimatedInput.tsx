...
export const AnimatedInput = (props: Props) => {
  ...
  // --- CORRECCIÓN AQUÍ ---
  // 1. Desestructuramos 'isTextArea' del resto de las props.
  //    'rest' contendrá todo lo demás (name, placeholder, type, etc.)
  const { isTextArea, ...rest } = props;

  return (
    <div className="form_field group relative overflow-hidden"> 
      {isTextArea ? (
        // 2. Pasamos solo '...rest' al <textarea>
        <textarea
          {...(rest as React.TextareaHTMLAttributes<HTMLTextAreaElement>)}
          className={clsx(commonClasses, "is-text-area min-h-60 resize-none")}
        />
      ) : (
        // 3. Pasamos solo '...rest' al <input>
        <input
